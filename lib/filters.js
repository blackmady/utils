...
exports.filters = {
    ...
    formatTime(d = new Date(), fmt = 'yyyy-MM-dd HH:mm:ss') {
        ...
        return utils_1.default.format(d, fmt);
    },
    /**格式化如下格式的时间201601080952134*/
    formatTime2(d, fmt = 'yyyy-MM-dd HH:mm:ss') {
        let nv = (d + '00000000000000').substr(0, 14);
        nv = nv.replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/, '$1/$2/$3 $4:$5:$6');
        return exports.filters.formatTime(new Date(nv), fmt);
    },
    /**格式化金额值，带逗号,u为单位字符串*/
    formatMoney: (v, l = 2, u = '') => isNaN(v) ? v : (v - 0).toFixed(l).replace(l ? /(?!^)(?=(\d{3})+\.\d*)/g : /(?!^)(?=(\d{3})+$)/g, ',') + (u || ''),
    /**格式化以分为单位的金额.*/
    formatMoney2: (v, l = 2, u = ' 元') => exports.filters.formatMoney((v || 0) / 100, l, u),
    /**格式化卡号，四位一空格*/
     ...
     * @return: string
     */
    zeroize: (num, len = 2, str = '0') => num.toString().padStart(len, str),
};
